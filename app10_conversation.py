...
class Conversation(Entity):
    def __init__(self, variables_object=None, **kwargs):
        ...
        self.question = Button(parent=self, text_origin=(-.5, 0), scale=(1, .1),
                               model=Quad(radius=.5, aspect=1/.1), text='Question')
        ...
        self.more_indicator = Entity(parent=self.question, model=Circle(
            3), position=(.45, -.4, -.1), rotation_z=180, color=color.azure, world_scale=.5, z=-1, enabled=False)
        ...
        self.answer_0 = Button(parent=self, text='answer_0', y=self.question.y-self.spacing-.025,
                               scale=(1, .075), text_origin=(-.5, 0), model=copy(self.button_model))
        self.answer_1 = Button(parent=self, text='answer_1', y=self.answer_0.y-self.spacing,
                               scale=(1, .075), text_origin=(-.5, 0), model=copy(self.button_model))
        self.answer_2 = Button(parent=self, text='answer_2', y=self.answer_1.y-self.spacing,
                               scale=(1, .075), text_origin=(-.5, 0), model=copy(self.button_model))
    ...
    def ask(self, node, question_part=0):
        ...
        if len(node.content) > 1 and self.question_part < len(node.content)-1:
            # question not finished, so don't show answer buttons
            if self.question_part < len(node.content):
                # print('question not finished')
                self.question_appear_sequence.append(
                    Func(setattr, self.more_indicator, 'enabled', True))
                return

        self.button_appear_sequence = Sequence()
        invoke(self.button_appear_sequence.start,
               delay=self.question_appear_sequence.duration)

        if not node.children:
            self.buttons[0].text = '*leave*'
            self.buttons[0].on_click = Func(setattr, self, 'enabled', False)
            self.button_appear_sequence.append(
                Func(setattr, self.buttons[0], 'enabled', True))

        for i, child in enumerate(answers):
            self.button_appear_sequence.append(Wait(i*.15))
            self.button_appear_sequence.append(
                Func(setattr, self.buttons[i], 'enabled', True))
            ...
            def on_click(node=child):
                ...
                if node.code and not node.code.startswith('if '):
                    try:
                        ...
                        print('failed executing code on node:',
                              node, 'code:', node.code, 'error:', e)
